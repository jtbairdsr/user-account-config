...
SEGMENT_START="$ULCORNER$HBAR"
SEGMENT_END="$HBAR$PROMPT_CHAR %{%k%f%}"
...
function get_lengths {
	...
	#                            doesn't exist. ${:-[string]} is a quick way to do variable related things to     #
	#                            fixed strings.                                                                   #
	...
	# ${#[varname]}          - Gives the length of the value of the variable. It appears that zsh handles the     #
	#                            pound sign before applying the (%) flag, so I had to nest the (%) flag in order  #
	#                            to get things to happen in the right order.                                      #
	#-------------------------------------------------------------------------------------------------------------#
	pwd_len=${#${(%):-%~}}
}
...
function context_content {
	...
	#                                                Username@hostname                                            #
	#-------------------------------------------------------------------------------------------------------------#
	#___________________________________________ Possible Customizations _________________________________________#
	...
	#-------------------------------------------------------------------------------------------------------------#
	#.................................................Explanation!................................................#
	# %([x]|[true]|[false]) - Specifies a ternary expression. The character following the [x] is arbitrary; the   #
	#                           same character is used to separate the text for the [true] result from that for   #
	#                           the [false] result. This separator may not appear in the [true], except as part   #
	#                           of a %-escape sequence. A ')' may appear in the [false] as '%)'. [true] and       #
	#                           [false] may both contain arbitrarily nested escape sequences, including further   #
	#                           ternary expressions.                                                              #
	# !                     - True if the shell  is  running  with  privileges.                                   #
	# @                     - literal '@' character                                                               #
	# %m                    - The hostname up to the first '.'. An integer may follow the '%' to specify how many #
	#                           components of the hostname are desired. With a negative integer, trailing         #
...
function path_content {
	...
	#   <...<    - Specifies truncation behaviour for the remainder of the prompt string. The '...' will be       #
	#                displayed in place of the truncated portion. Either '<' or '>' works, '<' truncates at the   #
	#                left, '>' truncates at the right.                                                            #
	#    %~      - Present working directory ($PWD), but if $PWD has a named directory as its prefix, that part   #
	#                is replaced by a '~' followed by the name of the directory. If it starts with $HOME, that    #
	#                part is replaced by '~'.                                                                     #
	#    %<<     - A truncation  with  argument zero marks the end of the range of the string to be truncated     #
	#                while turning off truncation from there on.                                                  #
	#-------------------------------------------------------------------------------------------------------------#
